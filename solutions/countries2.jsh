...
~~countries.stream().filter(country->country.getName().toFirstLetter==toLastLetter).forEach(System.out::println);
...
~~countries.stream().map(Country::getTranslations).summaryStatistics().count();

//7. Prints the names of countries in the ascending order of the number of timezones.
~~countries.stream().filter(country -> country.getName()!=null).flatMap(country -> country.getTimezones().stream()).sorted().forEach(System.out::println);
...
~~countries.stream().map(Country::getTimezones).sorted().count().getName().getPopulation();
...
~~countries.stream().equals(country.getArea()==null).flatMap(country -> country.getArea().stream()).forEach(System.out::println);
...
~~countries.stream().filter(country->country.getRegion()==Region.EUROPE).flatMap(country -> country.getTimezones().stream()).distinct().forEach(System.out::println);
